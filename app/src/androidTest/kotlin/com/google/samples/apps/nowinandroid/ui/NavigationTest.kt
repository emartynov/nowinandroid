 ...
 */
@HiltAndroidTest
val NavigationTest by testSuite {
    testFixture {
        object : JUnit4RulesContext() {
             ...
             */
            val postNotificationsPermission = rule(GrantPostNotificationsPermissionRule(), order = 1)
            ...
            val navigateUp by composeTestRule.stringResource(FeatureForyouR.string.feature_foryou_api_navigate_up)
            val forYou by composeTestRule.stringResource(FeatureForyouR.string.feature_foryou_api_title)
            val interests by composeTestRule.stringResource(FeatureSearchR.string.feature_search_api_interests)
            val sampleTopic = "Headlines"
            val appName by composeTestRule.stringResource(R.string.app_name)
            val saved by composeTestRule.stringResource(BookmarksR.string.feature_bookmarks_api_title)
            val settings by composeTestRule.stringResource(SettingsR.string.feature_settings_impl_top_app_bar_action_icon_description)
            val brand by composeTestRule.stringResource(SettingsR.string.feature_settings_impl_brand_android)
            val ok by composeTestRule.stringResource(SettingsR.string.feature_settings_impl_dismiss_dialog_button_text)
        }
    } asContextForEach {
        hiltRule.inject()

        test("firstScreen isForYou") {
         ...
         */
        test("navigationBar navigateToPreviouslySelectedTab restoresContent") {
            composeTestRule.apply {
         ...
         */
        test("navigationBar reselectTab keepsState") {
            composeTestRule.apply {
                // GIVEN the user follows a topic
                onNodeWithText(sampleTopic).performClick()
...
//    test("navigationBar reselectTab resetsToStartDestination") {
...
//    }
         ...
         */
        test("topLevelDestinations doNotShowUpArrow") {
            composeTestRule.apply {
        ...
        test("topLevelDestinations showTopBarWithTitle") {
            composeTestRule.apply {
        ...
        test("topLevelDestinations showSettingsIcon") {
            composeTestRule.apply {
        ...
        test("whenSettingsIconIsClicked settingsDialogIsShown") {
            composeTestRule.apply {
        ...
        test("whenSettingsDialogDismissed previousScreenIsDisplayed") {
            composeTestRule.apply {
         ...
         */
        test("homeDestination back quitsApp") {
            assertFailsWith<NoActivityResumedException> {
                composeTestRule.apply {
         ...
         */
        test("navigationBar backFromAnyDestination returnsToForYou") {
            composeTestRule.apply {
        ...
        // @Ignore — no direct testBalloon equivalent; test is disabled
        // test("navigationBar multipleBackStackInterests") { ... }

        test("navigatingToTopicFromForYou showsTopicDetails") {
            composeTestRule.apply {
